...
Original file is located at
    https://colab.research.google.com/drive/1q3oOq-_7EjdHOrAKO7sE5Vkg6M2CSWSX
...
# https://drive.google.com/file/d/1Z8Wh1LyjWDSefNuJqVVr3TgToaemNzN-/view?usp=sharing
#  !gdown --id '1Z8Wh1LyjWDSefNuJqVVr3TgToaemNzN-' --output data.zip
#  !unzip data.zip
#  !ls
...
def train_word2vec(x):
    ...
    model = word2vec.Word2Vec(x, size=250, window=5,
                              min_count=2, workers=12, iter=2, sg=1)
    return model
...
def training(batch_size, n_epoch, lr, model_dir, train, valid, model, device):
    ...
    criterion = torch.nn.BCELoss()  # 定義損失函數，這裡我們使用 binary cross entropy loss
    t_batch = len(train)
    v_batch = len(valid)
    ...
    for epoch in range(n_epoch):
        ...
        with torch.no_grad():
            ...
            for i, (inputs, labels) in enumerate(valid):
                # device 為 "cuda"，將 inputs 轉成 torch.cuda.LongTensor
                inputs = inputs.to(device, dtype=torch.long)
                # device 為 "cuda"，將 labels 轉成 torch.cuda.FloatTensor，因為等等要餵進 criterion，所以型態要是 float
                labels = labels.to(device, dtype=torch.float)
                outputs = model(inputs)  # 將 input 餵給模型
                outputs = outputs.squeeze()  # 去掉最外面的 dimension，好讓 outputs 可以餵進 criterion()
...
# 製作一個 model 的對象
model = LSTM_Net(embedding, embedding_dim=250, hidden_dim=5,
...
X_train, X_val, y_train, y_val = train_x[:
                                         100000], train_x[100000:], y[:100000], y[100000:]
print(X_train[:10])
print(y_train[:10])
# input('stop')
# print(type(X_train))
